angular.module("app_litsco").controller("controller_contact", [
  "$scope",
  "$state",
  "$http",
  "factory_meta",
  function($scope, $state, $http, factory_meta) {
    factory_meta.contact();

    angular
      .element(document.querySelector(".jumbo-div"))
      .css("backgroundImage", 'url("/img/jumbo_contact.jpg")');
    ...
    $scope.validateHuman = function(input, that) {
      if (input == 4) {
        angular
          .element(document.querySelector("#contact-submit-button"))
          .removeAttr("disabled");
      }
    };
    ...
    $scope.postMail = function(contact) {
      ...
      $http(req).then(
        function successCallback(response) {
          ...
          $(".form-contact-input-field, .form-contact-submit").prop(
            "disabled",
            true
          );
