angular.module("app_litsco").controller("controller_prods", [
  "$scope",
  "$state",
  "$stateParams",
  "$window",
  ...
  function (
    ...
    factory_meta.product(
      "LITSCO manufacturers " +
        $scope.productIdObj.productName +
        " in Queens, NY.  Download our data sheet for more information.",
      "LITSCO, streamline metal panels, streamline metal flashing, metal cladding, custom metal flashing, custom metal panels, roof panels, wall panels"
    );
    ...
    angular
      ...
      .css(
        "backgroundImage",
        'url("img/jumbo_' + $scope.productIdObj.jumboImg + '")'
      )
      .attr("alt", $scope.productIdObj.productName);
    ...
    $scope.calculateColumns = function () {
      ...
      for (
        var i = 0;
        i < $scope.productIdObj.features.length;
        i += itemsPerColumn
      ) {
        var col = {
          start: i,
    ...
    function updateGradient(color) {
      $("linearGradient#MyGradient #stop-0").attr(
        "stop-color",
        createShade(color, 0.2)
      );
    ...
    function updateSvgFill(hex) {
      $(".hover_color_change").attr(
        "fill",
        userAgentChromeOrFireFox ? "url(#MyGradient)" : hex
      );
    }
